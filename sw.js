...
// Version 2.0.0 - Searchable Dropdowns Update

const CACHE_NAME = 'reservasi-aula-bps-v2.0.0';
const urlsToCache = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('install', event => {
  console.log('Service Worker: Install event v2.0.0');
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        console.log('Service Worker: Caching files v2.0.0');
        return cache.addAll(urlsToCache);
      })
      .then(() => {
        console.log('Service Worker: All files cached v2.0.0');
...
self.addEventListener('activate', event => {
  console.log('Service Worker: Activate event v2.0.0');
  event.waitUntil(
    caches.keys().then(cacheNames => {
      ...
      console.log('Service Worker: Cache cleanup complete v2.0.0');
...
async function handleBackgroundSync() {
  ...
  console.log('Service Worker: Performing background sync for reservations v2.0.0');
...
self.addEventListener('push', event => {
  const options = {
    body: event.data ? event.data.text() : 'Notifikasi baru dari Reservasi Aula BPS',
    icon: './logo.png',
    badge: './logo.png',
    vibrate: [100, 50, 100],
    data: {
...
console.log('Service Worker: Script loaded successfully v2.0.0');